...
export const Terminal: React.FC<TerminalProps> = ({
  ...
  return (
    <div className={`flex flex-col space-y-3 font-mono select-none ${hasGlitch ? 'glitch-active' : ''}`}>
      {/* Heavy Industrial CRT Monitor Bezel Sitting on the Desk */}
      <div className="crt-bezel p-3 relative">
        {/* Four Corner Industrial Hex Screws */}
        <div className="absolute top-2 left-2 hex-screw" />
        <div className="absolute top-2 right-2 hex-screw" />
        <div className="absolute bottom-2 left-2 hex-screw" />
        <div className="absolute bottom-2 right-2 hex-screw" />

        {/* Industrial Stenciled Specification Badge */}
        <div className="flex items-center justify-between px-3 py-1 mb-2 border-b border-[#3d2020]/80">
          <div className="flex items-center space-x-2">
            <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse shadow-[0_0_6px_#10b981]" />
            <span className="text-[10px] font-black tracking-widest text-amber-500/90 font-typewriter uppercase">
              SERIAL NO. FT-991 // FORENSIC INTERCEPT UNIT
            </span>
          </div>

          <div className="flex items-center space-x-3">
            <span className="text-[9px] text-stone-500 font-typewriter hidden sm:inline">
              TERMINAL SPEC: CRT-V4.0 // AMBER PHOSPHOR
            </span>
            <button
              onClick={() => {
                audioFx.playClick();
              ...
              className="flex items-center space-x-1.5 px-2.5 py-0.5 rounded bg-[#1a0e0e] hover:bg-[#2e1515] text-amber-400 text-[10px] font-bold font-typewriter transition border border-[#4a2222]"
            >
        ...
        {/* CRT Monitor Screen with Scanlines and Phosphor Amber Glow */}
        <div className="crt-screen rounded-lg border-2 border-[#2b1212] overflow-hidden flex flex-col">
          {/* Mainframe Screen Status Subheader */}
          <div className="bg-[#140808] px-4 py-2 border-b border-[#2b1212] flex items-center justify-between text-xs relative z-30">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-amber-400/90 font-bold tracking-widest text-[11px] font-typewriter">
                {consoleHeader}
              </span>
            </div>
            ...
            <div className="flex items-center space-x-2">
              ...
              />
              <span className="text-[10px] font-bold text-amber-500/90 uppercase tracking-widest font-typewriter">
          ...
          {/* Heavy Metal Industrial Action Toolbar */}
          <div className="bg-[#120707] px-4 py-3 border-t-2 border-[#2b1212] flex items-center justify-between relative z-30">
            ...
            {/* Right: Test Intercept & Execute Warrant / Submit Evidence */}
            <div className="flex items-center space-x-3">
              <button
                onClick={handleRun}

import React from 'react';
import { X, ShieldCheck, FolderSearch } from 'lucide-react';
import { Clue } from '../../types/game';

interface ClueDrawerProps {
  isOpen: boolean;
  onClose: () => void;
  clues: Clue[];
}

export const ClueDrawer: React.FC<ClueDrawerProps> = ({ isOpen, onClose, clues }) => {
  if (!isOpen) return null;

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 backdrop-blur-sm p-4 font-mono">
      <div className="bg-theme-onyx border border-theme-darkBorder w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
        <div className="bg-theme-terminalInner px-5 py-3.5 border-b border-theme-darkBorderSubtle flex items-center justify-between">
          <div className="flex items-center space-x-2.5">
            <div className="w-8 h-8 rounded-lg bg-theme-bloodRed/30 border border-theme-scarlet/60 flex items-center justify-center">
              <ShieldCheck className="w-4 h-4 text-theme-scarlet" />
            </div>
            <h3 className="text-sm font-bold tracking-wide text-white uppercase">
              EVIDENCE LOCKER ({clues.length} CLUES ACQUIRED)
            </h3>
          </div>
          <button
            onClick={onClose}
            className="text-theme-textMuted hover:text-white p-1 rounded-lg hover:bg-theme-darkSurface transition"
          >
            <X className="w-5 h-5" />
          </button>
        </div>

        <div className="p-5 overflow-y-auto space-y-3.5 flex-1">
          {clues.length === 0 ? (
            <div className="text-center py-12 text-theme-textPlaceholder flex flex-col items-center">
              <FolderSearch className="w-10 h-10 mb-2 stroke-[1.5]" />
              <p className="text-xs text-theme-textMuted">No forensic clues collected yet.</p>
              <p className="text-[11px] text-theme-textPlaceholder mt-1">
                Execute queries and commands in the terminal to reveal case artifacts.
              </p>
            </div>
          ) : (
            clues.map((clue, idx) => (
              <div
                key={clue.id || idx}
                className="bg-theme-darkSurface border border-theme-darkBorder rounded-xl p-3.5 hover:border-theme-bloodRed transition relative overflow-hidden group shadow-sm"
              >
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded bg-theme-bloodRed/40 text-theme-scarlet border border-theme-scarlet">
                    {clue.category}
                  </span>
                  {clue.discoveredAt && (
                    <span className="text-[10px] text-theme-textMuted">{clue.discoveredAt}</span>
                  )}
                </div>
                <h4 className="text-xs font-bold text-white group-hover:text-theme-scarlet transition">
                  {clue.title}
                </h4>
                <p className="text-[11px] text-theme-textLight mt-1 leading-relaxed">
                  {clue.description}
                </p>
              </div>
            ))
          )}
        </div>

        <div className="bg-theme-terminalInner px-5 py-3 border-t border-theme-darkBorderSubtle flex justify-end">
          <button
            onClick={onClose}
            className="px-5 py-1.5 text-xs bg-theme-scarlet hover:bg-theme-scarletHover text-theme-white font-black rounded-lg transition shadow-md shadow-theme-scarlet/25"
          >
            Close Locker
          </button>
        </div>
      </div>
    </div>
  );
};
